...
type FastifyWebSocketConnection = WebSocketConnection | { socket: WebSocketConnection };

type ReadyServer = FastifyInstance & { setReady: (ready: boolean) => void };

const toWebSocket = (connection: FastifyWebSocketConnection): WebSocketConnection =>
  "socket" in connection ? connection.socket : connection;
...
export const buildServer = (options: ServerOptions = {}) => {
  ...
  app.after(() => {
    app.get("/ws", { websocket: true }, (connection: FastifyWebSocketConnection) => {
      const socket = toWebSocket(connection);
